...
export const commands: ChatCommands = {
	...
	/* custom version of the validator*/
	vtm(target, room, user, connection) {
		if (Monitor.countPrepBattle(connection.ip, connection)) {
			return;
		}
		...
		const options = {
		hostname: '217.182.65.153',
		port: 8081,
		path: 'validate/'+user.id+'/'+format.id,
		method: 'POST'
		}		  
		const req = http.request(options, res => {
			res.on('data',chunk => {
				if(res.statusCode==200){
				TeamValidatorAsync.get(format.id).validateTeam(user.battleSettings.team).then(result => {
					const matchMessage = (originalFormat === format ? "" : this.tr`The format '${originalFormat.name}' was not found.`);
					if (result.startsWith('1')) {
						connection.popup(`${(matchMessage ? matchMessage + "\n\n" : "")}${this.tr`Your team is valid for ${format.name}.`}`);
					} else {
					...
					connection.popup(`${this.tr`Your team was rejected for the following reasons:`}\n\n- ${JSON.parse(chunk)['message'].replace(/\n/g, '\n- ')}`);
				}
			})
